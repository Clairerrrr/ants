...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
       # if insect.is_ant:
        #    if insect.name!='Bodyguard' and self.ant is not None:
         #       assert self.ant is None, 'Two ants in {0}'.format(self)
          #  else:
           #     if insect.name == 'Bodyguard' and self.ant is None:
            #        self.ant=insect
             #   elif insect.name =='Bodyguard' and self.ant is not None:
              #      helper=self.ant
               #     self.ant=insect
                #    self.ant.contained_ant=helper
                #elif insect.name != 'Bodyguard' and self.ant is None:
                 #   self.ant=insect
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.label!=1 and self.ant.is_container==False:
                    self.ant=None
                    insect.place=None
                elif insect.label !=1 and self.ant.is_container==True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        #helper_place = self.place
        #bees = helper_place.bees
        #count=0
        #while helper_place != hive:
        #	if count>=self.max_range:
        #		return None
        #	elif bees is not None and count>=self.min_range:
        #		return random_or_none(bees)
        #	else:
        #		helper_place=helper_place.entrance
        #		bees=helper_place.bees
        #		count+=1
        #return None
        index=0
        ...
        while self.place is not hive and index<=self.max_range:
        	if not random_or_none(self.place.bees):
        		self.place=self.place.entrance
        		index+=1
        	else:
        		ending=random_or_none(self.place.bees)
        		self.place=ant_place
        		return ending
        self.place=ant_place
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.label==1:
            ...
            while hasattr(helper,'exit'):
                helper=helper.exit
                place.append(helper)
            for i in place:
                if i is not None and i.ant is not None:
                    if i.ant.is_container and i.ant.contained_ant != None and i.ant.contained_ant.armor_implemented ==False:
                        i.ant.contained_ant.damage*=2
                        i.ant.contained_ant.armor_implemented=True

                    if i.ant.armor_implemented==False:
                        i.ant.armor_implemented=True
                        i.ant.damage *=2
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0 and self.label==1:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    current_action = bee.action
    def new_action(colony):
        nonlocal duration
        duration -= 1
        if duration >=0:
            effect(current_action,bee)(colony)
            if duration == 0 and effect is make_scare:
                bee.scared = True
        else:
            current_action(colony)
